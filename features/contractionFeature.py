...
def contraction(email):
	data = email.body
	#this might change - but the idea is to convert the body of the email message into a list with each element representing one word
	data = list(data.split(' '))

	#using regex to capture time apostrophes in each word
	#the regex pattern - NOTE DAN, if you find another type of apostrophe, just stick it in anywhere in the square brackets after the forwardslash. It'll standardize it later
	...
	for word in data:
		...
		apostrophe = list(apostropheCapture.findall(word))
		...
		#NOTE - in the English language, double (and hell, even triple) contractions exist. But no matter how many contraction parts are present in a word, 
		#that would just increase the contraction_count by 1
		if apostrophe:
			if word.endswith("'t"):
				contraction_count += 1
	...
	total_msg_count = word_count(email.body)

	#percentage - the result
	contraction = "%.2f" % ((contraction_count / total_msg_count) * 100)
